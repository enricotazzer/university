public class Arrays {
    /* The mission of this class is to provide static methods that
     ...
     * is smaller or equal to each element in a_port[res+1,...,j]. MODIFY a.
     */
    private static int partition(int[] a, int i, int j) {
        ...
        while (true) {
            ...
            if (i > j) {
                return j;
            } else {// there are elements that we haven't scanned yet: let's swap the externswap(a, i, j);
                j--;
                i++;
            }
        }
